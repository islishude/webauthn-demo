...
async function getAttestationOptions(optionsRequest) {
  ...
  if (response.headers.get("Content-Type") !== "application/json") {
    throw new TypeError(
      "/attestation/options response header has unexpected Content-Type: " +
  ...
  if (typeof optionsResponse.excludeCredentials !== "undefined") {
    for (let i = 0; i < optionsResponse.excludeCredentials.length; i++) {
      optionsResponse.excludeCredentials[i].id = base64url.decode(
        optionsResponse.excludeCredentials[i].id
      );
    }
  }
...
async function sendAttestationResult(credential) {
  ...
  credentialResponse["clientDataJSON"] = base64url.encode(
    credential.response.clientDataJSON
  );
  credentialResponse["attestationObject"] = base64url.encode(
  ...
  if (response.headers.get("Content-Type") !== "application/json") {
    throw new TypeError(
      "/attestation/result response header has unexpected Content-Type: " +
        response.headers.get("Content-Type")
    );
  }
