...
async function getAssertionOptions(optionsRequest) {
  ...
  if (response.headers.get("Content-Type") !== "application/json") {
    throw new TypeError(
      "/assertion/options response header has unexpected Content-Type: " +
  ...
  if (typeof optionsResponse.allowCredentials !== "undefined") {
    for (let i = 0; i < optionsResponse.allowCredentials.length; i++) {
      optionsResponse.allowCredentials[i].id = base64url.decode(
        optionsResponse.allowCredentials[i].id
      );
    }
  }
...
async function sendAssertionResult(credential) {
  ...
  credentialResponse["authenticatorData"] = base64url.encode(
  ...
  if (response.headers.get("Content-Type") !== "application/json") {
    throw new TypeError(
      "/assertion/result response header has unexpected Content-Type: " +
        response.headers.get("Content-Type")
    );
  }
